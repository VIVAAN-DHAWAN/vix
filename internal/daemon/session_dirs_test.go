...
func TestAggregateSessionDirs_RanksByCountThenRecency(t *testing.T) {
	base := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)
	rec := func(cwd string, last time.Time, origin string) sessionRecord {
		return sessionRecord{CWD: cwd, StartedAt: base, LastRequestAt: last, Origin: origin}
	}
	recs := []sessionRecord{
		rec("/a", base.Add(1*time.Hour), ""),
		rec("/a", base.Add(2*time.Hour), ""),
		rec("/b", base.Add(5*time.Hour), ""),
		rec("/c", base.Add(9*time.Hour), ""), // most recent single-session dir
	...
	// /a's LastRequestAt reflects the newest of its two sessions.
	if got[0].LastRequestAt != base.Add(2*time.Hour).Format(time.RFC3339) {
		t.Errorf("/a LastRequestAt = %q, want newest session time", got[0].LastRequestAt)
	}
	for _, d := range got {
		if d.Path == "/vixdir" {
...
func TestAggregateSessionDirs_Empty(t *testing.T) {
	if got := aggregateSessionDirs(nil); len(got) != 0 {
		t.Errorf("empty input should yield no dirs, got %+v", got)
	}
}

func TestAggregateSessionDirs_FallsBackToStartedAt(t *testing.T) {
	base := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)
	recs := []sessionRecord{{CWD: "/only", StartedAt: base}}
	got := aggregateSessionDirs(recs)
