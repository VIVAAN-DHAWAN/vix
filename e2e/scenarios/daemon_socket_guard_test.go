...
// sessions list emptied and new sessions reported "daemon is not responding".
//
// asserts: (1) the conflicting vixd exits non-zero with an "already listening"
// message on the wire (its stderr) and (2) the original TUI session stays alive
...
func TestSecondDaemonRefusesSocketTakeover(t *testing.T) {
	...
	// One full turn so there is a live, working session before the conflict.
	...
	// The original session must be untouched: a follow-up turn still works.
